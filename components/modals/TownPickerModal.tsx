...
export default function TownPickerModal({
  ...
  const [expandedAreas, setExpandedAreas] = useState<Set<string>>(new Set(['Kanifing', 'Banjul']));
  ...
  const getZoneColor = (zone: string) => {
    switch (zone) {
      case 'zone1': return '#22C55E'; // Green
      case 'zone2': return '#3B82F6'; // Blue
      case 'zone3': return '#F59E0B'; // Yellow/Orange
      case 'zone4': return '#EF4444'; // Red
    ...
    return (
      ...
      >
        <View style={styles.townInfo}>
          <Text style={[styles.townName, isSelected && styles.townNameSelected]}>
            {town.name}
          </Text>
          <View style={styles.townMeta}>
            <Text style={styles.townArea}>{town.area}</Text>
            <View style={[styles.zoneBadge, { backgroundColor: getZoneColor(town.deliveryZone) + '20' }]}>
              <Text style={[styles.zoneBadgeText, { color: getZoneColor(town.deliveryZone) }]}>
                D{deliveryFee}
              </Text>
            </View>
          </View>
      ...
      <SafeAreaView style={styles.container}>
            ...
            <View style={styles.legendItems}>
              {zoneLegendData.map(({ key, fee }) => (
                <View key={key} style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: getZoneColor(key) }]} />
        ...
        {filteredTowns ? (
          ...
          <FlatList
            ...
            renderItem={({ item: [area, towns] }) => renderAreaSection(area, towns)}
            contentContainerStyle={styles.listContent}
          />
        )}
      </SafeAreaView>
